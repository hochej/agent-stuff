...
Chrome Profile 1 is configured for Oracle browser automation:
- **Profile**: `Profile 1` (josh.rockt@gmail.com)
- **Location**: `~/Library/Application Support/Google/Chrome/Profile 1/`

**Required flags for this machine:**
```bash
--browser-chrome-profile "Profile 1" --browser-cookie-wait 10s
```

The `--browser-cookie-wait 10s` is essential because macOS Keychain needs time to unlock and provide the cookie decryption key. If you've previously clicked "Always Allow" on the Keychain prompt, you may be able to reduce or omit this.

**Other available Chrome profiles** (if you need to switch):
| Profile | Email |
|---------|-------|
| Default | *(not signed in)* |
| Profile 1 | josh.rockt@gmail.com ✓ |
| Profile 2 | joschahoche@googlemail.com |
| Profile 3 | admin@hochej.com |
| Profile 4 | jhoche@noogler.google.com |
...
- Chrome profile: `--browser-chrome-profile "Profile 1"` (josh.rockt@gmail.com)
- Cookie wait: `--browser-cookie-wait 10s` (for macOS Keychain)
...
- Browser run (main path; long-running is normal):
  - `npx -y @steipete/oracle --engine browser --browser-chrome-profile "Profile 1" --browser-cookie-wait 10s -p "<task>" --file "src/**"`
